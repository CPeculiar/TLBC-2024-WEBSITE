...
document.addEventListener("DOMContentLoaded", () => {
  ...
  anyHealthCondition.addEventListener('change', function () {
    ...
    console.log(anyHealthCondition?.value)

    if (anyHealthCondition.value === 'true') {
      yourHealthConditionDiv.style.display = 'block';
    }else{
      yourHealthConditionDiv.style.display = 'none';
    }
  });
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      // console.log(bodyData); // Debugging statement
      ...
      // const queryParams = new URLSearchParams(formData1).toString();
